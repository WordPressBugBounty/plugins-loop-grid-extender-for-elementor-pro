<?php
namespace lgefep\feedback;

if ( ! defined( 'ABSPATH' ) ) {
	exit; }

class lgefep_feedback {

	private $plugin_url     = LGEFEP_PLUGIN_URL;
	private $plugin_version = LGEFEP_VERSION;
	private $plugin_name    = 'Loop Grid Extender for Elementor Pro';
	private $plugin_slug    = 'lgefep';
	...
	*/
	...
	function submit_deactivation_response() {
		if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), '_cool-plugins_deactivate_feedback_nonce' ) ) {
			...
			$plugin_initial = get_option('lgefep_initial_save_version');
			...
			$feedback_url      = LGEFEP_FEEDBACK_API .'wp-json/coolplugins-feedback/v1/feedback';
			$install_date 		= get_option('lgefep_install_date');
			$unique_key     	= '11';
